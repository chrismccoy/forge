...
polling, uncached HTTP, frontend option writes - and stays in step with the catalog. As
always the hits order the reading pass; they are not the audit.

Load testing note: only run the tests below against infrastructure you own or have written
authorization to test. Load-testing someone else's origin is an attack, whatever the intent.
