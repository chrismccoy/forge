...
│   (MySQL)       │   (the MySQL query cache was removed in 8.0)
└─────────────────┘
```
...
// Atomic ONLY with a persistent backend that implements ADD (Redis, Memcached).
// On the default in-process cache this is per-request, so any lock built on it
// is a no-op across concurrent PHP workers - check wp_using_ext_object_cache()
...
Requires a persistent object cache. Without one, `wp_cache_add()` cannot coordinate
across PHP workers and this pattern silently degrades to no locking at all.
...
function get_expensive_data() {
    ...
    // Another process is regenerating - wait, then retry a bounded number of
    // times. Never recurse unbounded here: under contention every worker that
    // misses the lock keeps re-entering, which is the stampede this pattern
    // exists to prevent.
    ...
    // Lock holder died or is slow - compute it ourselves rather than hang.
...
function get_data_with_stale() {
    ...
    // Schedule regeneration. Note this is only "background" in the sense that
    // this request does not wait for it - WP-Cron fires on a later request
    // unless DISABLE_WP_CRON is set and a system cron runs the queue. Core
    // de-duplicates identical events within a 10-minute window, so repeated
    // scheduling here does not pile up.
...
function set_cache_with_jitter($key, $data, $base_ttl) {
    ...
    // Cast the bounds: rand() with float arguments is deprecated in PHP 8.1+.
    $spread = (int) round($base_ttl * 0.1);
    $ttl    = $base_ttl + random_int(-$spread, $spread);
...
add_action('save_post', function($post_id, $post) {
    // Skip the noise: save_post also fires for autosaves, revisions, and every
    // row of a bulk edit. Without these guards the cache is flushed far more
    // often than the content actually changes.
...
The tag list is read, modified, and written back without a lock, so concurrent
writers can drop each other's keys. Acceptable when a missed invalidation only
means one stale entry until TTL; not acceptable when correctness depends on the
flush landing.
