...
'no_found_rows' => true,  // Skips the separate COUNT query WordPress runs for pagination
                          // (6.1 replaced SQL_CALC_FOUND_ROWS with that COUNT)

// Return only IDs when you don't need full post objects
'fields' => 'ids',

// ⚠️ NOT an optimization. 'suppress_filters' disables every plugin filter on the
// query - which is how object caching layers, multilingual plugins, and access
// control hook in. It is meaningful only for get_posts(), and even there it is a
// correctness decision, not a speed one. Flag it when you find it; do not add it.
...
function get_featured_posts() {
    ...
    if (false === $posts) {
        ...
        // Cache IDs, not post objects: smaller payload, and the posts are
        // re-read through the post cache so an edit is picked up without
        // waiting for this key to expire.
...
```sql
-- Composite index for one hot meta key/value pair.
-- Weigh it: every postmeta write now maintains another index, and the prefix
-- length has to cover the values you actually compare.
ALTER TABLE wp_postmeta ADD INDEX meta_key_value (meta_key, meta_value(50));
```

`wp_posts` already ships with `type_status_date (post_type, post_status, post_date, ID)`
in core, so do not add it - if a `post_type` + `post_status` + date query is not using
it, the cause is something else in the query (a function on the column, a wide `OR`, a
join that forces a different plan), and EXPLAIN will name it.

Adding indexes to core tables is a hosting-level change: it needs a migration path, it
is lost on some managed platforms, and on WordPress VIP it goes through the platform
team. Report the missing index as a finding; do not present `ALTER TABLE` as a fix the
plugin can ship.
